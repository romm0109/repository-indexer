...
export class EmbeddingService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.modelName =
      this.configService.get<string>('app.embedding.modelName') ||
      'text-embedding-3-small';
  }

  async embedDocuments(texts: string[]): Promise<number[][]> {
    const batchSize =
      this.configService.get<number>('app.embedding.batchSize') || 10;
  ...
  private async retryOperation<T>(operation: () => Promise<T>): Promise<T> {
    let lastError: any;
    const retries =
      this.configService.get<number>('app.embedding.retries') || 3;
    const baseDelay =
      this.configService.get<number>('app.embedding.retryDelay') || 1000;
